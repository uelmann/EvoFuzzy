"""Always-in top-decile long-only tranche book. Residual is cash. No hedge."""
...
def _pick_names(day: pd.DataFrame, top_pct: float) -> list[str]:
    ...
    k = max(1, int(np.ceil(float(top_pct) * len(g))))
    return g["symbol"].tolist()[:k]


def run_long_only_toppct(
    preds: pd.DataFrame,
    panel: pd.DataFrame,
    feat: pd.DataFrame,
    ...
    for i, dt in enumerate(dates[:-1]):
        ...
        names = _pick_names(day, top_pct) if not day.empty else []
        alphas[k] = (
    ...
    packed["pct_days_btc_held"] = float(n_btc_held / max(len(daily_btc_w), 1))
