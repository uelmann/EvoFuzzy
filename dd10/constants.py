"""Frozen constants for the DD10-LO target A/B (long-only top 10%).
...
IMPROVE_CRITERION = (
    "DD10-LO IMPROVES on A0-LO10 only if ALL of: "
    "(a) pooled OOS RankIC of DD10 predicted probability vs the continuous "
    "h=10 USDT-return / forward-average-drawdown ratio exceeds A0's RankIC vs the same ratio; "
    "(b) DD10 top-decile minus universe 10-day USDT simple return exceeds A0-LO10's; "
    "(c) DD10 long-only net Sharpe exceeds A0-LO10 net Sharpe; "
    "(d) the DD10 label-shuffle null is GREEN. "
    "This is an A/B target test, not a replacement for COMBO. No post-hoc adjustment."
)

VIABILITY_CRITERION = (
    "DD10-LO is VIABLE as a standalone long-only mandate only if ALL of: "
    "(a) full-period net Sharpe ≥ 0.50; (b) trailing-18m net Sharpe ≥ 0.00; "
    "(c) full-period total return > 0; (d) average deployed gross ≥ 0.15; "
    "(e) the DD10 label-shuffle null is GREEN. "
...
NULL_GATE = (
    ...
    "Skill passes if the real DD10 OOS RankIC exceeds the null 95th percentile on "
...
FALLBACK_RULE = (
    "If DD10's median best_iteration ≤ 1, refit with fixed 500 trees "
