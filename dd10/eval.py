"""Mechanical evaluation for DD10-LO."""
...
def improves(a0: dict, dd10: dict, ric_a0: float, ric_dd10: float, gap_a0: float, gap_dd10: float, null_verdict: str) -> dict:
    null_ok = str(null_verdict).upper() == "GREEN"
    pass_ric = bool(np.isfinite(ric_dd10) and np.isfinite(ric_a0) and ric_dd10 > ric_a0)
    pass_gap = bool(np.isfinite(gap_dd10) and np.isfinite(gap_a0) and gap_dd10 > gap_a0)
    sh_d = float(dd10.get("net_sharpe_full", float("nan")))
