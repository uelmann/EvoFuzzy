"""DD10-LO report, JSON, and equity chart."""
...
from dd10.constants import IMPROVE_CRITERION, VIABILITY_CRITERION
from dd10.eval import _as_utc, _sharpe, cagr_maxdd, window_slice
...
def plot_equity(
    ...
    series = [(a0, "A0-LO10 (frozen scores)"), (dd10, "DD10-LO (P top-decile ret/ADD)")]
    ...
    axes[0].set_title("DD10-LO vs A0-LO10 (long-only top 10%)")
...
def write_report(
    ...
    *,
    ...
    text = f"""# DD10-LO — P(top 10% of ret/avgDD) vs frozen A0 (long-only)
...
| DD10 | {_fmt(ric_dd10_ratio.get('mean_ic'))} | {_fmt(ric_dd10_ratio.get('icir'))} | {_fmt(ric_dd10_ratio.get('nw_tstat'))} | {ric_dd10_ratio.get('n_days')} | {_fmt(ric_dd10_simple.get('mean_ic'))} | {_fmt(gap_dd10.get('mean_gap'), 5)} | {_pct(gap_dd10.get('pct_gap_pos'))} | {_fmt(gap_dd10.get('nw_t'))} |
...
Elapsed seconds: {_fmt(extra.get('elapsed_sec'), 1)}. GPU used: false. Scheduled jobs created: false. DD10 fallback 500 trees: {extra.get('used_fixed')}. Mean label rate: {_fmt(extra.get('mean_label_rate'), 4)}.
"""
