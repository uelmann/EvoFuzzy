"""
DD10-LO — binary P(top 10% of ret/avgDD) vs frozen A0, long-only top-decile book.
...
Usage: modal run --detach dd10_pipeline.py
"""
...
image = (
    ...
    .add_local_python_source("baseline", "dd10")
    ...
    .add_local_file("reports/dd10_addendum.md", remote_path="/root/dd10_addendum.md")
)
...
def train_fold_job(payload: dict) -> dict:
    ...
    from dd10.constants import SEED, YCOL
...
def run_dd10() -> dict:
    ...
    from baseline.seedutil import seed_everything
    from dd10.book import run_long_only_toppct
    from dd10.constants import (
        DEATH_CONVENTION,
        FALLBACK_RULE,
        FEAT_PATH,
    ...
    from dd10.labels import add_dd10_labels
    from dd10.report import plot_equity, write_report
    ...
    addendum = Path("/root/dd10_addendum.md").read_text()
    ...
    print("[HB] DD10-LO BACKTEST ONLY; zero GPU; COMBO untouched; A0 artifacts read-only", flush=True)
    ...
    out = Path(OUT_ROOT)
    if out.resolve() != Path("/data/quant/dd10").resolve():
        raise RuntimeError(f"refusing to write outside /data/quant/dd10: {out}")
    ...
    feat = add_dd10_labels(feat, panel, horizon=HORIZON)
    if YCOL not in feat.columns or YCOL_RATIO not in feat.columns:
        raise RuntimeError("dd10 labels missing on working frame")
    ...
    used_fixed = False
    dd_iters = _iters(main_metas)
    if dd_iters and float(np.median(dd_iters)) <= 1.0:
        used_fixed = True
        print("[HB] DD10 median best_iteration ≤ 1; refitting with fixed 500 trees", flush=True)
        mcfg = dict(model_cfg)
    ...
    if not parts:
        raise RuntimeError("empty DD10 preds")
    pred_dd = last_fold_wins(pd.concat(parts, ignore_index=True))
    pred_dd["date"] = pd.to_datetime(pred_dd["date"], utc=True)
    pred_dd_path = pred_dir / "lgbm_dd10_h10.parquet"
    pred_dd.to_parquet(pred_dd_path, index=False)
    volume.commit()
    ...
    book_dd = _prep_book(pred_dd)
    ric_a0_ratio = pooled_rankic(book_a0, YCOL_RATIO)
    ric_dd_ratio = pooled_rankic(book_dd, YCOL_RATIO)
    ric_a0_simple = pooled_rankic(book_a0, YCOL_SIMPLE)
    ric_dd_simple = pooled_rankic(book_dd, YCOL_SIMPLE)
    gap_a0 = top_minus_universe(book_a0, "score", YCOL_SIMPLE)
    gap_dd = top_minus_universe(book_dd, "score", YCOL_SIMPLE)

    print("[HB] running A0-LO10 and DD10-LO books", flush=True)
    raw_a0 = run_long_only_toppct(book_a0, panel, feat, pit40, horizon=HORIZON, funding=funding, variant="a0_lo10")
    raw_dd = run_long_only_toppct(book_dd, panel, feat, pit40, horizon=HORIZON, funding=funding, variant="dd10_lo")
    a0_sum = summarize_book(raw_a0)
    dd_sum = summarize_book(raw_dd)
    verdict = viable(dd_sum, null.get("verdict", "PARKED-NO-SKILL"))
    improve = improves(
        a0_sum,
        dd_sum,
        float(ric_a0_ratio.get("mean_ic", float("nan"))),
        float(ric_dd_ratio.get("mean_ic", float("nan"))),
        float(gap_a0.get("mean_gap", float("nan"))),
        float(gap_dd.get("mean_gap", float("nan"))),
        null.get("verdict", "PARKED-NO-SKILL"),
    )
    print(f"[HB] improve={improve.get('verdict')} viable={verdict.get('verdict')}", flush=True)

    idx = dd_sum.get("daily_ret")
    ...
    chart_path = chart_dir / "dd10_equity.png"
    ...
    extra_blob = {
        ...
        "seed_gate": seed_gate,
        "construction": (
            f"Frozen A0 Huber scores vs new binary LightGBM on P(top-decile R/ADD); "
            ...
            f"n_a0={len(pred_a0)} n_dd10={len(pred_dd)}; mean_y={mean_label:.4f}; "
            f"write root={OUT_ROOT}; frozen features sha256={frozen_feat_sha}."
        ),
    }
    md_path = rep_dir / "dd10_report.md"
    text = write_report(
        md_path,
        frozen_hash=calc,
        a0_book=a0_sum,
        ...
        ric_dd10_ratio=ric_dd_ratio,
        ric_a0_simple=ric_a0_simple,
        ric_dd10_simple=ric_dd_simple,
        gap_a0=gap_a0,
        gap_dd10=gap_dd,
    ...
    summary = {
        ...
        "dd10_book": {k: v for k, v in dd_sum.items() if k != "daily_ret"},
        "used_fixed": used_fixed,
        "n_folds": len(folds),
        "mean_label_rate": mean_label,
        "elapsed_sec": time.time() - t_pipe,
    ...
    (rep_dir / "dd10_report.json").write_text(json.dumps(_jsonable(summary), indent=2, default=str))
    (root / "reports").mkdir(parents=True, exist_ok=True)
    (root / "charts").mkdir(parents=True, exist_ok=True)
    (root / "reports" / "dd10_report.md").write_text(text)
    (root / "reports" / "dd10_report.json").write_text((rep_dir / "dd10_report.json").read_text())
    if chart_path.exists():
        (root / "charts" / "dd10_equity.png").write_bytes(chart_path.read_bytes())
    volume.commit()
    print(
    ...
    return {
        ...
        "sharpe_dd10": verdict.get("sharpe_full"),
...
def main():
    print("[local] starting DD10-LO (CPU, backtest-only, COMBO untouched)...", flush=True)
    summary = run_dd10.remote()
    ...
    for remote, name, kind in [
        ("reports/dd10_report.md", "dd10_report.md", "reports"),
        ("reports/dd10_report.json", "dd10_report.json", "reports"),
        ("charts/dd10_equity.png", "dd10_equity.png", "charts"),
        ("dd10/reports/dd10_report.md", "dd10_report.md", "reports"),
        ("dd10/reports/dd10_report.json", "dd10_report.json", "reports"),
        ("dd10/charts/dd10_equity.png", "dd10_equity.png", "charts"),
    ]:
        dest = art / kind / name
