"""Vol-matched null for Variant A. Folds {5,15,21,24} × 15."""
...
from nfn_va.constants import NULL_K_EXCEED, NULL_REPLICATES, NULL_STOUFFER_Z, NULL_TRAIN_SEED
from nfn_va.data import PackedPanel
from nfn_va.train import train_one_fold
...
def assemble_null(cells_by_metric: dict, real_keys: dict) -> dict:
    ...
    return {
        "name": "variant_a_vol_matched_null",
...
def run_null_cell(
    ...
    pred, meta = train_one_fold(
        ...
        n_inits=1,  # house null: one init, seed-42 craft otherwise identical
    )
    ...
    sm = fold_tail_pack(pred, labeled, close, btc_id, "score")
    return {
        "fold_id": int(fold.fold_id),
        "shuffle_seed": int(shuffle_seed),
...
def cells_from_replicates(folds: list[FoldSpec], recs: list[dict], real: dict[int, dict]) -> dict:
    ...
    for fold in folds:
        ...
        cells["tail_ic_top"].append(_fold_cell(fold, [r.get("tail_ic_top") for r in rows], real, "tail_ic_top"))
        cells["overlap"].append(_fold_cell(fold, [r.get("overlap") for r in rows], real, "overlap"))
        cells["monster"].append(_fold_cell(fold, [r.get("monster") for r in rows], real, "monster"))
