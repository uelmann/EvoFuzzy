...
- GPU (operator override 2026-08-16): 12×A10G fold-parallel (`PHASE5_GPU_MAX_CONTAINERS`). List price `PHASE5_GPU_USD_PER_HOUR = 1.10`. 4×H100 was tried first; per-batch time matched sequential A10G so H100 was stopped. Prior A10G sequential hours plus the short H100 probes are sunk cost counted toward the same **USD 80** cap. Stage A (CPU download) does not count toward the cap. Architecture / seeds / folds / criteria unchanged.
- GPU leftover (operator override 2026-08-16 14:45 UTC): the detached 12-wide wave wrote 58/74 jobs then **judged incomplete**. Real worker-file spend was **$62.40 / $80** (`aborted=False`). The leftover 6 jobs (seed-43 fold 15 + null fold-21 shuffles 106–110) never trained: each worker billed `12 × (now − orchestrator_t0)` even after the map shrank, so `17.41 + 12×4.25h×1.10 + $8 reserve ≥ $80` aborted them at start. Leftover finish uses **completed worker hours + this-wave n_jobs × wave elapsed**, cap still $80. LIVE/PARKED criterion text unchanged.
