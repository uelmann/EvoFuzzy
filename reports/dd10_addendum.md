# DD10-LO — freeze addendum

**Status:** FROZEN before results. Parallel A/B test: same A0 features and LightGBM hyperparameters, new **binary** target, long-only top-decile book.
**Reference books:** COMBO v2.0-combo-final, SPREAD-LS BOOK-HYBRID, LONG-TIDE. **UNTOUCHED.**
**Scope:** backtest and analysis only. New Modal app `quant-dd10`. Writes only under `/data/quant/dd10/`. CPU only. Zero GPU. No live components.

This is **not** a product. This is **not** a retrain of frozen A0 Huber. Frozen scores, `features_labeled.parquet`, COMBO, SPREAD-LS, and LONG-TIDE are **read-only**. Kronos / MASTER / VQ / MoE / GP miners are out of scope.
...
> Bias: every judged fold's null mean RankIC must satisfy |mean| ≤ 2·(SD / √R). Skill passes if the real DD10 OOS RankIC exceeds the null 95th percentile on **both** judged folds. Failure = PARKED (CONTAMINATED if bias fails, PARKED-NO-SKILL if bias passes and skill fails). No override, no retest with different folds.
...
> If DD10's median best_iteration ≤ 1, refit with fixed 500 trees (A0 h=10 fallback). The judged book uses the refit. This rule is frozen before results.
...
- **DD10 (treatment):** new LightGBM **binary** classifier. Score = predicted P(y=1). Same hparams otherwise: `num_leaves` 31, `lr` 0.03, `min_data_in_leaf` 200, `feature_fraction` 0.8, bagging 0.8/1, `lambda_l2` 1.0, seed 42, n_estimators 3000, patience 100, early-stop mean daily RankIC vs the **binary** label.
...
- Forward path average drawdown: for k = 1..h, `peak_k = max(close[t], …, close[t+k])`, `dd_k = 1 − close[t+k] / peak_k` (0 if peak is 0/NaN). `ADD = mean_k dd_k`.
- Ratio: `ratio = R / (ADD + 1e-4)`.
...
This is **not** the frozen residual `y_h10`. The residual column is renamed `y_resid_h10` on the working copy and is unused for training.
...
- **Long the top 10%** of that day's PIT top-40 names by score (predicted P for DD10; Huber score for A0-LO10). `k = max(1, ceil(0.10 · n_finite))`. No τ, no second head, no min-3 skip.
...
- Writes under `/data/quant/dd10/` and copies the report/chart into the volume `reports/` and `charts/` sync paths.
- Does not use GPU.
