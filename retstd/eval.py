"""Mechanical evaluation for RETSTD-LO."""
...
def summarize_book(res: dict) -> dict:
    ...
    out["pct_days_btc_held"] = float(res.get("pct_days_btc_held", float("nan")))
    out["daily_ret"] = net
    return out
...
def viable(book: dict, null_verdict: str) -> dict:
    ...
    ok = bool(pass_full and pass_trail and pass_total and pass_gross and null_ok)
    return {
        "viable": ok,
...
def improves(a0: dict, retstd: dict, ric_a0: float, ric_retstd: float, gap_a0: float, gap_retstd: float, null_verdict: str) -> dict:
    null_ok = str(null_verdict).upper() == "GREEN"
    pass_ric = bool(np.isfinite(ric_retstd) and np.isfinite(ric_a0) and ric_retstd > ric_a0)
    pass_gap = bool(np.isfinite(gap_retstd) and np.isfinite(gap_a0) and gap_retstd > gap_a0)
    sh_d = float(retstd.get("net_sharpe_full", float("nan")))
    sh_a = float(a0.get("net_sharpe_full", float("nan")))
    pass_sh = bool(np.isfinite(sh_d) and np.isfinite(sh_a) and sh_d > sh_a)
    ok = bool(pass_ric and pass_gap and pass_sh and null_ok)
    return {
        "improves": ok,
        ...
        "pass_null": null_ok,
        "ric_a0": ric_a0,
        "ric_retstd": ric_retstd,
        "gap_a0": gap_a0,
        "gap_retstd": gap_retstd,
        "sharpe_a0": sh_a,
        "sharpe_retstd": sh_d,
        "null_verdict": null_verdict,
    }
...
def top_minus_universe(
    ...
    """Per-date (top-bucket mean − universe mean) of ycol."""
    daily = []
...
def ew_topn_simple(panel: pd.DataFrame, universe: pd.DataFrame) -> pd.Series:
    ...
    for dt, g in uni.groupby("date", sort=True):
        ...
        if dt not in r.index:
            continue
        names = [s for s in g["symbol"].tolist() if s in r.columns]
        if not names:
            continue
        row = r.loc[dt, names]
...
def btc_bh_simple(panel: pd.DataFrame) -> pd.Series:
    ...
    r = p.set_index("date")["close"].pct_change(fill_method=None)
    return r.dropna()
