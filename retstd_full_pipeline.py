"""
RETSTD-FULL — same R/std target, 3000 trees, no early stop.
...
Usage: modal run --detach retstd_full_pipeline.py
"""
...
image = (
    ...
    .add_local_file("reports/retstd_full_addendum.md", remote_path="/root/retstd_full_addendum.md")
)
...
def run_retstd_full() -> dict:
    ...
    from retstd.full_constants import (
        FULL_TREES,
        IMPROVE_CRITERION,
        NULL_GATE,
        OUT_ROOT,
    ...
    addendum = Path("/root/retstd_full_addendum.md").read_text()
    for needle in (IMPROVE_CRITERION, VIABILITY_CRITERION, NULL_GATE, DEATH_CONVENTION, TRAIN_RULE):
        if needle not in addendum:
            raise RuntimeError("Addendum missing a verbatim frozen statement")
    print("[HB] RETSTD-FULL BACKTEST ONLY; 3000 trees; no early stop; zero GPU; COMBO untouched; A0 artifacts read-only", flush=True)
    ...
    out = Path(OUT_ROOT)
    if out.resolve() != Path("/data/quant/retstd_full").resolve():
        raise RuntimeError(f"refusing to write outside /data/quant/retstd_full: {out}")
    ...
    print(f"[HB] folds={len(folds)} h={HORIZON} trees={FULL_TREES} early_stop=off", flush=True)
    model_cfg = dict(cfg["model"])
    model_cfg["objective"] = "binary"
    model_cfg["fixed_n_estimators"] = int(FULL_TREES)
    ...
    statuses = [(int(m.get("fold_id", -1)), (m.get("meta") or {}).get("status"), (m.get("meta") or {}).get("best_iteration"), (m.get("meta") or {}).get("early_stop_mode")) for m in main_metas]
    if len(rs_iters) != len(folds) or any(int(x) != int(FULL_TREES) for x in rs_iters):
        raise RuntimeError(f"expected every fold to train {FULL_TREES} trees; statuses={statuses}")
    if any(mode != f"fixed_{FULL_TREES}" for _, st, _, mode in statuses):
        raise RuntimeError(f"expected early_stop_mode=fixed_{FULL_TREES}; statuses={statuses}")
    print(f"[HB] all folds trained {FULL_TREES} trees; no early stop; iters={rs_iters}", flush=True)
    ...
    pred_rs_path = pred_dir / "lgbm_retstd_full_h10.parquet"
    ...
    chart_path = chart_dir / "retstd_full_equity.png"
    if isinstance(idx, pd.Series) and len(idx) and isinstance(a0r, pd.Series) and len(a0r):
        plot_equity(
            a0r,
            idx,
            ew if len(ew) else None,
            btc if len(btc) else None,
            chart_path,
            treat_label="RETSTD-FULL (3000 trees, P top-decile ret/std)",
            title="RETSTD-FULL vs A0-LO10 (long-only top 10%)",
        )
    ...
    extra_blob = {
        ...
        "seed_gate": seed_gate,
        "short_name": "RETSTD-FULL",
        "improve_criterion": IMPROVE_CRITERION,
        "viability_criterion": VIABILITY_CRITERION,
        "train_note": f"trees={FULL_TREES} early_stop=off iters={rs_iters}",
        "train_rule": TRAIN_RULE,
        "death_convention": DEATH_CONVENTION,
        "construction": (
            f"Frozen A0 Huber scores vs binary LightGBM P(top-decile R/STD); "
            f"EVERY fold trained exactly {FULL_TREES} trees; no early stop; "
            f"last-fold-wins; n_folds={len(folds)}; "
    ...
    md_path = rep_dir / "retstd_full_report.md"
    text = write_report(
        md_path,
        frozen_hash=calc,
        a0_book=a0_sum,
        ...
        extra=extra_blob,
        product_name="RETSTD-FULL",
        treat_row_name="RETSTD-FULL",
        heading="RETSTD-FULL — P(top 10% of ret/std), 3000 trees, no early stop",
        mandate_line="long-only top 10% of PIT top-40 by score. Residual is cash. No hedge. Training: 3000 trees, no early stop.",
    )
    summary = {
        "frozen_sha256": calc,
        "gpu_used": False,
    ...
    (rep_dir / "retstd_full_report.json").write_text(json.dumps(_jsonable(summary), indent=2, default=str))
    (root / "reports").mkdir(parents=True, exist_ok=True)
    (root / "charts").mkdir(parents=True, exist_ok=True)
    (root / "reports" / "retstd_full_report.md").write_text(text)
    (root / "reports" / "retstd_full_report.json").write_text((rep_dir / "retstd_full_report.json").read_text())
    if chart_path.exists():
        (root / "charts" / "retstd_full_equity.png").write_bytes(chart_path.read_bytes())
    volume.commit()
    print(
...
def main():
    print("[local] starting RETSTD-FULL (3000 trees, no early stop) (CPU, backtest-only, COMBO untouched)...", flush=True)
    summary = run_retstd_full.remote()
    ...
    for remote, name, kind in [
        ("reports/retstd_full_report.md", "retstd_full_report.md", "reports"),
        ("reports/retstd_full_report.json", "retstd_full_report.json", "reports"),
        ("charts/retstd_full_equity.png", "retstd_full_equity.png", "charts"),
        ("retstd_full/reports/retstd_full_report.md", "retstd_full_report.md", "reports"),
        ("retstd_full/reports/retstd_full_report.json", "retstd_full_report.json", "reports"),
        ("retstd_full/charts/retstd_full_equity.png", "retstd_full_equity.png", "charts"),
    ]:
        dest = art / kind / name
